...
- **`ios/App/App/PrivacyInfo.xcprivacy`** is the machine-readable form of the "Data Not Collected"
  answer, wired into Copy Bundle Resources. Every array is empty, which was verified rather than
  assumed: no required-reason API is reachable from app code or from any of the seven plugins, and
  the journal persists through WebKit localStorage rather than NSUserDefaults (@capacitor/preferences,
  which would force a `CA92.1` UserDefaults declaration, is deliberately not a dependency). Capacitor
  and Cordova ship their own manifests inside their frameworks. Rewrite this file, privacy.html and
  the App Store Connect answers together when Supabase lands.
