...
because it is true regardless of notification permission.

**Journal export goes through the share sheet, not `<a download>` (fixed 2026-09-12).** The export
row built a blob URL and clicked a synthetic anchor, which works in a browser and does **nothing at
all** in a WKWebView: Capacitor's iOS runtime implements no `WKDownloadDelegate` and no download
decision handler (verified by grep against its Swift sources), so the feature was tested in the
browser and shipped dead. That made both a Settings row and a sentence in the store description
untrue, which is a Guideline 2.1 problem as much as an honesty one. `exportJournal` in
[SettingsSheet.tsx](src/screens/library/SettingsSheet.tsx) now branches: native writes the JSON to
`Directory.Cache` via @capacitor/filesystem and hands the URI to @capacitor/share, so the user gets
Save to Files / Mail / AirDrop; web keeps the anchor download, which is what browsers actually
support. Cache rather than Documents is deliberate: it is a handoff, not a file the app keeps.
That is the reason for the `C617.1` entry in PrivacyInfo.xcprivacy. Plugin count is now **9**. Fonts (Archivo, Archivo Black)
...
**`brand/` at the repo root, deliberately NOT under `public/`** (moved 2026-09-12): nothing in `src/`
or `index.html` ever referenced them, so sitting in the Vite public root they were copied wholesale
into the IPA, 1.3M of dead weight out of a 1.9M web bundle, including a `wordmark.svg` that still
spelled POURFECT inside a binary submitted as 15GRMS. That wordmark is deleted (the live wordmark is
text, not an asset). The only two shipped brand files are `public/favicon.svg` and
`public/apple-touch-icon.png`. Alternate icons per palette (`UIApplication.setAlternateIconName`)
...
- Install and run: `xcrun simctl install booted <path>/App.app && xcrun simctl launch booted com.alxnhfr.fifteengrams`
- **Target shape (hardened 2026-09-12, pre-submission audit).** `TARGETED_DEVICE_FAMILY = "1"`,
  iPhone only: the Capacitor template shipped `"1,2"`, which declares iPad support, and App Store
  Connect then refuses the submission until a 13 inch iPad screenshot set exists. There is no iPad
  layout to screenshot: the shell is capped at `maxWidth: 440` with no width breakpoint anywhere in
  ...
  that stops every single upload from parking in "Missing Compliance" and blocking TestFlight.
  The stale project-level `CODE_SIGN_IDENTITY = "iPhone Developer"` is removed so automatic signing
  resolves the identity instead of a scripted archive picking up a development certificate.
  actool still emits an `AppIcon76x76@2x~ipad.png` because the icon is a single `universal` entry;
  that is inert, `UIDeviceFamily` is what App Store Connect reads and it is `[1]`.
- **`ios/App/App/PrivacyInfo.xcprivacy`** is the machine-readable form of the "Data Not Collected"
  answer, wired into Copy Bundle Resources. `NSPrivacyCollectedDataTypes` is empty and true: there is
  no network call, so there is nowhere for data to go, and the journal persists through WebKit
  localStorage rather than NSUserDefaults (@capacitor/preferences, which would force a `CA92.1`
  UserDefaults declaration, is deliberately not a dependency). It declares exactly one required-reason
  API, `NSPrivacyAccessedAPICategoryFileTimestamp` with reason **C617.1**, brought in by
  @capacitor/filesystem (`IONFileStructures+Converters.swift` reads ctime/mtime when reporting file
  attributes). The API is compiled into the binary whether or not the app calls `stat()`, so it is
  declared. Required-reason APIs are orthogonal to data collection; this does not weaken the answer.
  Capacitor and Cordova ship their own manifests inside their frameworks, but @capacitor/share and
  @capacitor/filesystem ship none, which is why their usage lands here. Rewrite this file,
  privacy.html and the App Store Connect answers together when Supabase lands.
- **Build numbers.** `CURRENT_PROJECT_VERSION` and `MARKETING_VERSION` are set per-configuration at
  the TARGET level, so each lives in two places in `project.pbxproj`. Deliberately not hoisted to the
  project level: Xcode's General tab writes target-level values, so hoisting just means Xcode puts
...
- **`App.xcscheme` is committed** at `ios/App/App.xcodeproj/xcshareddata/xcschemes/`, with the Archive
  ...
  exists for the CLI export path and is unused by the Organizer flow.
